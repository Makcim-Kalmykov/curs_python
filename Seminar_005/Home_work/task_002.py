...
def serch_winner(motion, current):
    while current > 28:
        if motion % 2 != 0:
            player = 1
            motion = 1
        ...
        print(f"Игрок {player}: введите кол-во конфет:")
    ...
    if motion == 2:
            print(f"Осталось {current} конфет. Победил игрок № 2 ")
    else:
            print(f"Осталось {current} конфет. Победил игрок № 1 ")


numbers = open_filse()
num = random.randint(1, 2)
serch_winner(num, numbers)